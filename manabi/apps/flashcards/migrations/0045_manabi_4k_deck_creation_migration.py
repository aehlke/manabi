...
def make_ruby(kana, kanji):
    ...
    if kana[-4:] == kanji[-4:] == '(する)':
        return make_ruby(kana[:-4], kanji[:-4]) + '（する）'
    elif kana[:3] == kanji[:3] == '(お)':
        return '（お）' + make_ruby(kana[3:], kanji[3:])
    ...
    if (
        (
            ...
            or any((not is_kana(c) and c not in ['/', '／']) for c in kana)
        )
        ...
        if not kanji:
            return kana
        print(kana[:-4], kanji[:-4], (kana[:-4] == kanji[:-4] == '(する)'))
    ...
    suffix = commonprefix([kana.split('／', 1)[0][::-1], kanji[::-1]])[::-1]
